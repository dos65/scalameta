...
class ScalametaParser(input: Input)(implicit dialect: Dialect) { parser =>
  ...
  class NewTokenIterator(
    ...
    override def next(): Token = {
      ...
      //println(s"return ${curr.token} ${curr.token.getClass()} $sepRegions ${hashCode()}")
      curr.token
    }
    ...
    private def nextToken(
      ...
      // println(s"--curr ${curr.token} ${curr.token.getClass()} $sepRegions ${hashCode()}")
      ...
      def currRef: TokenRef = TokenRef(curr, currPos, currPos + 1)
      ...
      if (isTrailingComma) {
        ...
        if (curr.is[LeftParen]) (RegionParen :: sepRegions, currRef)
          ...
          sepRegions match {
            case x :: xs if x.isIndented => (xs, mkOutdent)
        ...
        def lastWhitespaceToken = {
          ...
          TokenRef(out, lastNewlinePos, lastNewlinePos + 1)
        }
        ...
        if (dialect.allowSignificantIndentation) {
          if (lastNewlinePos != -1 && next != null && !isLeadingInfixOperator(next)) {
            ...
            if (needOutdent)
              ...
              (RegionIndent(nextIndent, prev.is[KwMatch]) :: sepRegions, mkIndent)
          ...
          if (canProduceLF) {
            if (isLeadingInfixOperator(next)) {
              ...
              (sepRegions, TokenRef(token, lastNewlinePos, lastNewlinePos + 1))
            }
          } else {
            nextToken(prevPos, nextPos, sepRegions)
          }
    ...
    override def tokenPos: Int = curr.pos

    override def token: Token = curr.token

    override def fork: TokenIterator =
      new NewTokenIterator(scannerTokens, sepRegions, curr, prevPos)
